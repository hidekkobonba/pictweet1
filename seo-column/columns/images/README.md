...
### 作成予定の図解（小鼻縮小ページ用・枠だけ設置済み＝画像を入れると自動表示）
> `kobana-shukusho-fukuoka.html` に枠を設置済み。下記ファイル名で入れると図解が表示されます（未設置の間はプレースホルダ表示）。推奨：横長（例 1200×675／中に説明文を入れる場合は figcaption なしでOK）。
| ファイル | 何を描くか |
|---|---|
| `illust-kobana-bui.jpg` | ①部位の名称：鼻尖（鼻先）／小鼻（鼻翼）／鼻の穴（鼻孔）／鼻孔縁（穴のフチ） |
| `illust-kobana-method.jpg` | ②どこを切る：内側法＝鼻の穴の中／外側法＝小鼻の外側のキワ／内外側法＝両方 |
| `illust-bikoen.jpg` | ③鼻孔縁挙上：鼻の穴のフチ（鼻孔縁）を切って持ち上げる |
